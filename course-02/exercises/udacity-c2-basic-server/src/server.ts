...
(async () => {
  ...
  app.get("/cars/", async (req: Request, res: Response) => {
    ...
    const cars = cars_list.filter(itm => !make || itm.make === make);
  ...
  app.get("/cars/:id", async (req: Request, res: Response) => {
    ...
    const car = cars_list.filter(itm => itm.id === +id);
    if (!car || car.length === 0) {
      return res.status(404).send(`No car found with the Id #${id}!`)
    }
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    ...
    if (!id || !make || !type || !model || !cost) {
      res.status(400).send("id, make, type, model, and cost are required!")
    }
